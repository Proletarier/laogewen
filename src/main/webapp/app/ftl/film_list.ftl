...
<head>
    ...
    <link href="css/vod_list.css" rel="stylesheet">
    <script src="../public/js/jquery-1.12.0.min.js"></script>
    <script src="../public/js/bootstrap.min.js"></script>
    <script src="../main/js/main.js"></script>
    <script src="js/vod_list.js"></script>
			    ...
			    <#list (page) as val>
				 <#if (val.filmType)??>
                    <li style="display:none;">
                        <a class="p" title="${val.filmName}" href="/app/vod/${val.filmType}/${val.createDate?string("yyyy/MMdd")}/${(val.filmID)?c}.html.html" target="_blank">
                            <img src="${val.titleImg}" alt="">
                        </a>
                        <div class="info">
                            <h2>
                                <a href="/app/vod/${val.filmType}/${val.createDate?string("yyyy/MMdd")}/${(val.filmID)?c}.html.html" title="${val.filmName}" target="_blank">${val.filmName}</a>
                            </h2>
                            <p><i>更新：${val.createDate?string("yyyy-MM-dd")}</i></p>
                            <p><i>类型：${typeCodeMeaning}</i></p>
                            <p><i>撸量：${val.clickAmount}</i></p>
                            <span>
                                <a href="/app/vod/${val.filmType}/${val.createDate?string("yyyy/MMdd")}/${(val.filmID)?c}.html.html" target="_blank">观看</a>
