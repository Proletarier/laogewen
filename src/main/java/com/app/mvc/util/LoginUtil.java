...
public class LoginUtil {
    ...
    private final static String DEFAULT_ONE = "@6CFB18E2#iLpYs8Tb";
    ...
    public static void logout(HttpServletRequest request,HttpServletResponse response){
        CookieUtil.setCookie(request,response,USER_NAME, "",0);
    }
    ...
    public static String generateUserCookie(HttpServletRequest request, User user) {
        ...
        String mac = IPUtils.getMACAddress(ip).replaceAll("-", "");
        //设置cookieuser类
        CookieUser cookieUser = CookieUser.builder().userId(user.getUserId()).userName(user.getUserName()).ip(ip).mac(mac).lastLogin(System.currentTimeMillis()).build();
        //base64加密,并将=使用默认字符替换
        String encodeBase = Base64.encodeBase64String((JacksonUtil.toJSon(cookieUser).getBytes())).replaceAll("=", DEFAULT).replaceAll("\r|\n","");
        //反转
